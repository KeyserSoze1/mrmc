...
public:
  ...
  virtual int           discover_find_devices_custom(uint32_t target_ip, uint32_t device_type, uint32_t device_id, struct hdhomerun_discover_device_t result_list[], int max_count)
    { return::hdhomerun_discover_find_devices_custom(target_ip, device_type, device_id, result_list, max_count); };
  virtual struct hdhomerun_device_t* device_create_from_str(const char *device_str, struct hdhomerun_debug_t *dbg)
    { return ::hdhomerun_device_create_from_str(device_str, dbg); }
  virtual void          device_destroy(struct hdhomerun_device_t *hd)
    { hdhomerun_device_destroy(hd); }
  virtual int           device_stream_start(struct hdhomerun_device_t *hd)
    { return ::hdhomerun_device_stream_start(hd); }
  virtual uint8_t*      device_stream_recv(struct hdhomerun_device_t *hd, size_t max_size, size_t* pactual_size)
    { return ::hdhomerun_device_stream_recv(hd, max_size, pactual_size); }
  virtual void          device_stream_stop(struct hdhomerun_device_t *hd)
    { hdhomerun_device_stream_stop(hd); }
  virtual int           device_set_tuner_channel(struct hdhomerun_device_t *hd, const char *channel)
    { return ::hdhomerun_device_set_tuner_channel(hd, channel); }
  virtual int           device_set_tuner_program(struct hdhomerun_device_t *hd, const char *program)
    { return ::hdhomerun_device_set_tuner_program(hd, program); }
  virtual int           device_set_tuner_from_str(struct hdhomerun_device_t *hd, const char *tuner_str)
    { return ::hdhomerun_device_set_tuner_from_str(hd, tuner_str); }
  virtual void          device_set_tuner(struct hdhomerun_device_t *hd, unsigned int tuner)
